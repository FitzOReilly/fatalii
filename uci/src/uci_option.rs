...
use search::{
    alpha_beta::{
        ...
        LATE_MOVE_PRUNING_BASE, LATE_MOVE_PRUNING_FACTOR, LATE_MOVE_PRUNING_MIN_DEPTH,
...
pub const OPTIONS: [UciOption; 14] = [
    UciOption {
        name: "Hash",
    ...
    UciOption {
        ...
        r#type: OptionType::Spin(SpinProps {
            default: LATE_MOVE_PRUNING_BASE as i64,
            min: 0,
            max: 16,
    ...
    UciOption {
        ...
        r#type: OptionType::Spin(SpinProps {
            default: LATE_MOVE_PRUNING_FACTOR as i64,
            min: 0,
            max: 8,
    ...
    UciOption {
        name: "late-move-pruning-min-depth",
        r#type: OptionType::Spin(SpinProps {
            default: LATE_MOVE_PRUNING_MIN_DEPTH as i64,
            min: 1,
            max: 8,
            fun: set_late_move_pruning_min_depth,
        }),
    },
    UciOption {
...
#[allow(dead_code)]
fn set_late_move_pruning_min_depth(engine: &mut Engine, depth: i64) -> String {
    engine.set_search_params(SearchParamsEachAlgo::AlphaBeta(AlphaBetaParams {
        late_move_pruning_min_depth: Some(depth as usize),
        ..Default::default()
    }));
    format!("late-move-pruning-min-depth set to {depth}")
}
