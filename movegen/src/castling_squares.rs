    ...
    pub fn new(queen_rook_file: File, king_file: File, king_rook_file: File) -> CastlingSquares {
        ...
        for file in min_kingside_file..=max_kingside_file {
            cs.white_kingside.non_attacked |=
                Bitboard::from(Square::from((File::from_idx(file), Rank::R1)));
            cs.black_kingside.non_attacked |=
                Bitboard::from(Square::from((File::from_idx(file), Rank::R8)));
        }
        ...
        for idx in min_kingside_file..=max_kingside_file {
            ...
            cs.white_kingside.non_blocked |= Bitboard::from(Square::from((file, Rank::R1)));
            cs.black_kingside.non_blocked |= Bitboard::from(Square::from((file, Rank::R8)));
        }
        ...
        for idx in min_queenside_file..=max_queenside_file {
            ...
            cs.white_queenside.non_blocked |= Bitboard::from(Square::from((file, Rank::R1)));
            cs.black_queenside.non_blocked |= Bitboard::from(Square::from((file, Rank::R8)));
        }
